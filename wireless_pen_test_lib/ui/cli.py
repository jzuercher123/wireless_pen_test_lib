...
from wireless_pen_test_lib.core.config.protocols import register_scanners
from wireless_pen_test_lib.core.config.protocols import register_exploits
...
def initialize_coreframework():
    from wireless_pen_test_lib.core import CoreFramework  # Import inside the function to avoid circular imports
...
def configure(ctx, settings):
    ...
    if not settings:
        ...
        config_path = os.path.join(str(core.config_manager.config_dir), 'configs.yaml')
